...
export function History({ sessions, currentSessionId }: { sessions: SessionSummary[]; currentSessionId?: string }): React.JSX.Element {
  ...
  const filtered = useMemo(() => sessions.filter((session) => session.title.toLowerCase().includes(query.toLowerCase())), [sessions, query]);
  return (
    <main className="page history-page">
      <div className="page-heading"><div><h2>历史对话</h2><p>{sessions.length} 个 OpenCode 会话</p></div><button className="primary" onClick={() => post({ type: "new-session" })}>＋ 新增对话</button></div>
