...
> 文档状态：0.3.2 已实现并通过构建、单测、扩展激活、VSIX 与 OpenCode 1.18.25 配置重载验证
> 生成日期：2026-08-29  
...
0.3.2 修正：OpenCode 1.18.25 的 `/config` 更新接口会生成启动时不加载的 `config.json`，因此 RainEye 改为直接、安全地修改官方 `opencode.json/opencode.jsonc`，并支持旧文件迁移。MCP 增加编辑/删除/重连菜单；Skill 支持多选文件夹和编辑/重载/移除菜单；历史列表只显示含消息的会话；界面仅保留 VS Code 原生视图标题栏命令。
